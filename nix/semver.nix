...
let
  ...
  comparePrerelease =
    pre1: pre2:
    if pre1 == null && pre2 == null then
      0
    else if pre1 == null then
      1 # pre1 (no prerelease) > pre2 (prerelease)
    else if pre2 == null then
      -1 # pre1 (prerelease) < pre2 (no prerelease)
    else
  ...
  compareSemVer =
    ...
    let
      parse =
        ...
        in
        if m == null then
          builtins.throw "Invalid SemVer: ${v}"
        else
          {
    ...
    if cMajor != 0 then
      cMajor
    else if cMinor != 0 then
      cMinor
    else if cPatch != 0 then
      cPatch
    else
      comparePrerelease s1.prerelease s2.prerelease;
